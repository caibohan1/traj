...
public:
    TrajectoryPlanner() : Node("trajectory_planner")
    {
        ...
        RCLCPP_INFO(this->get_logger(), "Clean Decoupled Circle Planner (ENU) with Error Logging Started.");
...
private:
    ...
    void init_visual_path()
    {
        ...
        for (double t = 0; t <= T; t += 0.1) {
            ...
            // 叠加相对起飞原点
            pose.pose.position.x = start_x_ + R * std::sin(omega_max * t);
            pose.pose.position.y = start_y_ + R - R * std::cos(omega_max * t); 
    ...
    void timer_callback()
    {
        ...
        const double T_takeoff = 5.0; // 纯起飞耗时
        const double T_land    = 5.0; // 纯降落耗时
