...
#include <std_msgs/msg/float64_multi_array.hpp> // 【新增】用于发送扩展的 Snap 和角加速度
#include <chrono>
#include <cmath>
...
public:
    TrajectoryPlanner() : Node("trajectory_planner")
    {
        ...
        RCLCPP_INFO(this->get_logger(), "Figure-8 Trajectory Planner (with Analytical Snap) Started.");
    }

private:
    rclcpp::Publisher<px4_msgs::msg::TrajectorySetpoint>::SharedPtr setpoint_pub_;
    rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr path_pub_; 
    rclcpp::Publisher<std_msgs::msg::Float64MultiArray>::SharedPtr ext_setpoint_pub_; // 【新增】
    ...
    void init_visual_path()
    {
        ...
        for (double t = 0; t <= T; t += dt) {
            ...
            pose.pose.position.x = A * std::sin(omega * t);
            pose.pose.position.y = B * std::sin(2.0 * omega * t);
    ...
    void timer_callback()
    {
        ...
        std_msgs::msg::Float64MultiArray ext_msg{}; // 【新增】
        ...
        double B = 1.0;  // Y 轴振幅
        // ================= X / Y 轴保持 8 字形连续运动 =================
        double px = A * std::sin(omega * t);
        double py = B * std::sin(2.0 * omega * t);
        
        double vx = A * omega * std::cos(omega * t);
        double vy = 2.0 * B * omega * std::cos(2.0 * omega * t);
        
        double ax = -A * omega * omega * std::sin(omega * t);
        double ay = -4.0 * B * omega * omega * std::sin(2.0 * omega * t);
        
        double jx = -A * omega * omega * omega * std::cos(omega * t);
        double jy = -8.0 * B * omega * omega * omega * std::cos(2.0 * omega * t);

        // 【新增】X 和 Y 轴 Snap (对 Jerk 求导)
        double sx = A * std::pow(omega, 4) * std::sin(omega * t);
        double sy = 16.0 * B * std::pow(omega, 4) * std::sin(2.0 * omega * t);
